...
> **日期**：2026-07-15（细化修订）；**实现记录**：2026-07-16 见 **§13**
> **状态**：讨论稿 + **P0 本地代码已落地**（未部署远端）；契约以 §13.3 修复后为准
...
> - [260612-实现清单.md](./260612-实现清单.md)（任务归属以本稿 §10–§11 / §13 为准）
> - 前端现状：[frontend/README.md](../../../frontend/README.md)（已接 Obs / fixture，非纯 Mock）

**本文结构**：§1–§3 决策与架构 → **§4 前端推进路线** → **§5–§9 各模块改动** → §10 依赖与分期 → §11 拍板项 → **§13 实现记录、审查修复与残留风险**。
...
## 13. 实现记录（2026-07-15 / 07-16 本地落地）

> **范围**：按本文 P0 在本地完成代码接入；**未同步到远端服务器**。
> **状态**：可本地联调；下文含审查发现并已修复的契约 bug。

### 13.1 已落地模块对照

| 模块 | 路径 / 落点 | 交付要点 |
|------|-------------|----------|
| **Server Obs** | `uenv-server/src/obs/` | ingest / merge / `obs.db` / axum REST+SSE；`ObsConfig::from_env`；`emit` 非阻塞队列 |
| **启动** | `uenv-bridge/core/src/main.rs` | 与 trajectory 并列 `tokio::spawn(obs::serve)`；`state.obs` OnceLock |
| **控制面 hook** | `episode.rs` / `ports.rs` / `result_finalizer.rs` / `control_plane.rs` / `prelude_and_guards.rs` | Submit / Dispatch / StreamReport 转译 / 终态 / Register；超时/取消亦 emit |
| **Bridge** | `obs_client.py` + `verl_agent_loop.py` + `core.rs` metadata | `RUN_*` HTTP；`training_run_id` 写入 payload/metadata |
| **Worker** | `executor.rs` | P0 无新通道；StreamReport 已带 `correlation_id`/`worker_id`/`report_type` |
| **Frontend** | `lib/types` / `store` / `aggregation-client` / `use-run-stream` / `training-console` | 脱 Mock；无 URL 时 fixture；只读观测（start/stop 禁用） |
| **文档 / 脚本** | `uenv-server/README.md`、`frontend/README.md`、`scripts/obs/smoke_obs_http.sh` | 环境变量与冒烟步骤 |

### 13.2 前端接入要点（实现后）

| 项 | 约定 |
|----|------|
| 环境变量 | `VITE_AGGREGATION_BASE_URL` → Server Obs（如 `http://127.0.0.1:50053`）；`VITE_DEFAULT_RUN_ID` / `?run=` |
| Fixture | 未配置 base URL 时自动演示，不依赖后端 |
| SSE | `GET /api/v1/runs/{id}/stream`；`event:` = 类型，`data:` = **裸** JSON（见 §13.3） |
| Token | REST 用 `Authorization: Bearer`；EventSource 用 `?token=`（浏览器限制） |
| 快照 | 纯前端深拷贝；不中断 SSE |
...
| 严重度 | 问题 | 影响 | 修复 |
|--------|------|------|------|
| **高** | SSE `data` 曾序列化为 `SsePayload` 标签包装 `{"type","data"}`，前端按裸 `ChainState`/`StateDelta` 解析 | 接真实 Obs 时状态全空/字段错乱 | `http.rs`：按 `event:` 发**裸载荷** |
| **高** | `WorkerView.active_episodes`：Server 为 `string[]`，前端曾为 `number` | 活跃 Worker 计数恒假 | 类型与 fixture / 控制台改为数组长度判断 |
| **中** | `WorkflowEdge`：Server `from`/`to`，前端曾 `from_node_id`/`to_node_id` | 边数据对不齐 | 前端对齐 Server |
| **中** | `StateDelta.cursor`：Server 用 `last_event_id` 等，前端曾用 `event_id`/`seq` | 重连游标丢失 | `DeltaCursor` 与 `apply-delta` / `use-run-stream` 对齐 |
| **中** | 每次 Worker 心跳都推 `state_delta` | SSE/UI 可能被刷爆 | merge：`WORKER_HEARTBEAT` 只改内存、**不** fan-out |
| **低** | `last_event_id` 查询参数 Server 未消费 | 重连靠连接时 `full_state`，无增量补齐窗口 | P0 可接受；P1 再做 cursor 重放 |
| **低** | 超时/取消路径与 `complete_episode_result` 各自 emit 终态 | 当前路径互斥，未见双发；若日后合流需去重 | 文档标注；靠 `event_id` 幂等兜底 |
| **高** | `StateDelta.entity_key`：Server 对 episode/step/worker 发复合 patch（含 `workflow`/`tree`），前端按 key 只合进子对象或丢弃 `step:` | 直播增量下 UI 不更新 / EpisodeView 被污染 | merge 统一 `entity_key=run` + 顶层 patch；前端对「误标子实体但带 run 级字段」做防御合并 |
| **中** | 树 `root_id`=`training_run_id`，根节点 `node_id`=`run:{id}` | `buildUiTree` 按 `root_id` 查找常失败 | Server 空态与 normalize/UI 统一 `root_id=run:{id}` |
| **中** | SSE `onError` 与 EventSource 原生重连叠加计数 | 易瞬间打满失败阈值误切 Mock | 单次错误只调度一次手动重连 |
| **中** | Bridge `RUN_*` 与样本 `metadata.training_run_id` 解析不一致 | 同一 batch 事件落到不同 run | `run_batch` 把 batch 级 id 写入各样本 `extra_info` |

### 13.4 已知残留风险（刻意未改行为 / P0 可接受）

> 二次审查（2026-07-16）确认；**行为未改**，联调与 P1 规划时需知情。

| 风险 | 现状 | 影响 | 建议 |
|------|------|------|------|
| **`RUN_CLOSED` 后拒收事件** | Bridge `run_batch` 的 `finally` 发 `RUN_CLOSED`；merge 对已 closed run 除 `RUN_*` 外一律 `RejectedClosed` | 正常 await 路径下终态多已落盘；若存在异步尾巴（迟到 StreamReport / 终态 emit），episode 完成态可能丢更新 | P0：依赖 batch 内 await 完成后再关；P1：关闭宽限期，或 closed 后仍接受「终态类」事件 |
| **SSE `last_event_id` 服务端未消费** | 查询参数与浏览器 `Last-Event-ID` 均未做增量重放；每次连上先推当前 `full_state` | 正确但浪费带宽；断线窗口内无「只补 delta」 | 见 §13.3 低优先级；P1 cursor 重放 |
| **联调 run id 默认 `_orphan`** | 前端未带 `?run=` / 未配 `VITE_DEFAULT_RUN_ID` 时盯 `_orphan`；无 `training_run_id` 的 Server 事件也归 `_orphan` | 真实训练事件在业务 run id 上时，UI 看起来「一直空/只有占位」 | 联调必须显式 `?run=<id>` 或与 Bridge 相同的 `VITE_DEFAULT_RUN_ID` / `UENV_TRAINING_RUN_ID` |

### 13.5 本地验证

```bash
# Obs 单测
PROTOC=… cargo test -p uenv-server obs::tests

# 前端类型
cd frontend && npx tsc --noEmit

# Obs HTTP 冒烟（需 adapter-core 已起 Obs）
./scripts/obs/smoke_obs_http.sh
```
...
| # | 结论（本次实现采用） |
|---|----------------------|
| 1 端口 | 独立 `UENV_OBS_HTTP_LISTEN` 默认 `:50053` |
| 2 控制 | P0 **只读观测**；start/stop UI 禁用 |
| 3 Worker step | StreamReport → Server 转译 |
| 4 `training_run_id` | Bridge 写入；缺失归 `_orphan` |
| 5 队列满 | `try_send` 失败丢弃 + `dropped_events` 计数 |

### 13.7 尚待（非本次阻塞）

- Obs 重启后前端 cursor 增量补齐（现：重连发 `full_state`）；含上表 `last_event_id` 重放
- `RUN_CLOSED` 宽限期 / 终态例外（上表残留风险）
- P1：日志 / Metrics Tab、回放、前端 start/stop
- Worker 可选 Obs HTTP 细粒度补发
- 生产 CORS / token / 反代 SSE 缓冲关闭

### 13.8 各模块 Mock / 占位策略（防缺上报报错）

| 层 | 缺实现 / 未上报时行为 |
|----|------------------------|
| **Frontend** | 无 Obs URL → Fixture；Obs 不可达连续失败 → **Mock 回落** Fixture；已连接但无业务事件 → 空工作流占位 + 提示（不抛错）；`normalizeChainState` 容错缺字段/旧字段名 |
| **Server Obs** | `ensure_run` 始终返回合法 `ChainState`（默认工作流节点）；`POST .../seed` 注入 mock worker+episode；`UENV_OBS_AUTO_MOCK=1` 时 GET/SSE 空 run 自动 seed |
| **Bridge** | 未设 `UENV_OBS_URL` 时 emit no-op；HTTP 失败只打日志，**永不抛到训练路径** |
| **Worker** | P0 无 Obs 客户端；不上报也不影响前端（靠 Server 转译或占位） |

前端顶栏：`Fixture 演示` = 纯离线；`Mock 回落` = 曾配置 Obs 但已回落本地占位。
