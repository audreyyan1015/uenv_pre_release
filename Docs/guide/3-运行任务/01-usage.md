...
3. [获取轨迹](./12-trajectory.md)：使用 `uenv trajectory` 查询、下载或批量导出 UEnv 保存的轨迹。

具体任务的输入、变量取值、命令和验收集中在[案例库](./02-cases.md)。接入新强化学习框架时才需要阅读[自定义框架接入指南](../4-接入强化学习框架/01-custom-framework.md)。
