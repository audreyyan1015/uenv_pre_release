...
当前发布为 VeRL v0.7.1 提供正式用户入口，使用方法见[以 VeRL 为例接入 UEnv](../4-接入强化学习框架/02-verl.md)。其他强化学习框架需要按[自定义强化学习框架接入](../4-接入强化学习框架/01-custom-framework.md)实现转换层。

下面的 `uenv train` 命令已经封装 VeRL 适配器、数据转换和 GPU 容器配置，普通训练用户不需要自己处理框架接入协议。
...
需要查询或下载训练轨迹时，继续阅读[获取轨迹](./12-trajectory.md)。VeRL 的接入流程见[以 VeRL 为例接入 UEnv](../4-接入强化学习框架/02-verl.md)；任务案例见[训练案例](./02-cases.md#强化学习训练)。
