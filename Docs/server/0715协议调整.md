...
`git diff --stat` 显示 tracked 文件当前约 `47 files changed, 3563 insertions(+), 7178 deletions(-)`。这个统计来自当时的阶段性工作树，不包含 untracked 新增目录和文件，例如 `uenv-server/src/service/`、`uenv-server/src/agent_pool/`、`uenv-server/src/trajectory/`、OpenHands 新生成的 `common_pb2.py/episode_pb2.py`。

主要变化可以分为八类：

1. 协议层从 JSON/metadata/info 兜底迁到 typed proto 字段。
2. clean proto 已覆盖正式 `/home/uenv/proto`，正式 proto 现在就是唯一协议来源。
...
- `proto/CLEAN_PROTO_FIELDS.md`
- `proto/REMAINING_CLEAN_MIGRATION.md`
...
- `/home/uenv/proto` 已切到 clean proto；Python bridge 和 OpenHands runtime protobuf 已重新生成。
...
- clean proto dry-run：在临时副本中验证 clean proto 覆盖 `proto/` 后，`cargo check -p uenv-server`、`cargo check -p uenv-worker`、`cargo check -p uenv-adapter-core` 均通过。
...
- 旧临时 clean proto 目录已删除，clean proto 文档已迁入 `/home/uenv/proto`。
