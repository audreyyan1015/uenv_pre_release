...
> **文档版本**：v1.4  
> **依据方案**：[UEnv 设计 PRD v7.2](./uenv-design-prd-v7.2.md)（内置环境矩阵 §3.2.5；MathEnv 验证型）  
> **适用范围**：Worker Pool 层（Layer 2 环境执行层）的实现与协作边界  
> **最后更新**：2026-05-31  
> **v1.4 变更**：Phase 0 环境收敛由「单 gsm8k env_type」调整为 **通用 MathEnv（`env_type=math`）**；GSM8K 降为 math 下首个 dataset/benchmark 实现；与 UEnvHub seed、Bridge、Server 调度语义对齐（见 §9）  
...
```
2026-05-25T12:00:00.123456+08:00 INFO uenv.worker.episode trace_id=t-abc episode_id=ep-abc123 worker_id=worker-1 env_type=math msg="Episode completed" duration_ms=523 reward=1.0
```
...
```json
{
  ...
  "env": { "types": ["math"], "plugin_dir": "./plugins", "backend": "process" },
...
- 环境实例可用 **其他语言** 实现（如 Python MathEnv；GSM8K 为 math 下首个 benchmark 后端）
...
| 语言 | 单语言先闭环（建议 Rust 或 Python 二选一落地 **MathEnv**） | 多语言并行 |
...
初期目录约定（PRD v7.2 对齐：`env_type=math` 为 L1 调度键；dataset 在 payload 内区分 benchmark）：

```
plugins/
└── math/                      # env_type=math（MathEnv 通用计算/验证环境）
    ├── manifest.yaml          # env_type=math, version, backends, config_schema
    ├── backends/
    │   └── gsm8k/             # 首个 benchmark 后端（原 plugins/gsm8k 逻辑迁入）
    ├── plugin.toml
    └── (二进制或启动脚本)
```

**过渡期**：现有 `plugins/gsm8k/`（`env_type=gsm8k`）在 M7 实机验收中仍可用；收敛后 Worker 注册 **`math` only**，并对 `gsm8k` 请求做 **deprecated alias**（日志告警 + 路由到 math 插件），直至 e2e/fixture 全部切换完毕。
...
UENV_MOCK_EPISODE_FIXTURE=./fixtures/math/episode_001.pb
...
### 8.5 Mock 数据要求（MathEnv / GSM8K benchmark）

初期 fixture 至少覆盖：

- **`env_type = "math"`**（L1 调度键；与 UEnvHub seed、Bridge、Server 一致）
- **`payload.dataset = "gsm8k"`**（benchmark 标识；可选，默认 gsm8k 后端）
...
> **历史**：M7 实机（2026-05-30）使用 `env_type=gsm8k` 通过；属 Phase 0 临时实现。新 fixture 与联调 **以 `math` 为准**（见 §9.4 迁移清单）。
...
## 9. Phase 0 范围：MathEnv（`env_type=math`）通用计算环境

> **依据**：[UEnv 设计 PRD v7.2 §3.2.5](./uenv-design-prd-v7.2.md) 内置环境矩阵 — **MathEnv** 为验证型环境，承载数学推理、可验证规则奖励等 **一切计算类** Phase 0 任务；GSM8K 等 benchmark **不是** 独立 `env_type`，而是 math 环境下的 dataset 路由。

### 9.1 环境收敛（跨层语义）

| 层级 | Phase 0 目标 `env_type` | GSM8K 角色 | 说明 |
|------|---------------------------|------------|------|
| **UEnvHub** | `math` | manifest `tags` / `examples` 含 gsm8k | seed 已存在（`uenv-hub-core/src/seed.rs`） |
| **Bridge / VeRL** | `math` | `data_source=gsm8k` → `env_type=math` | VeRL `task_to_env_type.gsm8k: math`（PRD §8.1 ROLL+MathEnv） |
| **UEnv Server** | 按 `math` 调度 | — | `supported_env_types` 含 `math` 的 Worker |
| **Worker** | 注册并预热 **`math`** 实例池 | gsm8k 后端在插件内 | 单 MathEnv 插件，多 benchmark 可共存 |
| **插件 IPC** | 不变（Proto/UDS） | `payload.dataset` 选后端 | L2 协议与 L1 `env_type` 解耦 |

**设计原则**：

1. **`env_type` = 环境能力类**（math / code / agent），与 PRD「3 核心自研」一致。  
2. **`dataset` / `benchmark` = payload 字段**，区分 GSM8K、MATH-500 等数据源，**不占用** L1 调度键。  
3. **一个 Worker 进程一套 math 预热池**，所有计算类 Episode 共享 MathEnv 实例；不同 benchmark 仅影响插件内判分/模板，不影响 Server 路由表。

### 9.2 MathEnv 插件交付物

| 交付物 | 说明 |
|--------|------|
| `plugins/math/manifest.yaml` | `env_type: math`；`config_schema` 对齐 Hub math manifest |
| `plugins/math/backends/gsm8k/` | 首个 benchmark：规则 Reward（原 gsm8k 插件逻辑） |
| 单轮执行模式 | 问题 → 答案 → `rule_reward`（方案 §4.4「单轮」） |
| 测试 | 单元测试 + MockScheduler + **math** fixture |

**payload 最小契约**（L1 `EpisodeRequest.payload` JSON）：
...
**reward_config**（L1 独立字段，不变）：

```json
{ "type": "rule_reward", "target": "20" }
```

### 9.3 验收标准（Worker 层）

1. Mock / Remote 模式下 **`env_type=math`** 完整跑通：Register → Dispatch → Execute → Report  
2. GSM8K benchmark（`dataset=gsm8k`）在 math 插件内 reward=1.0 可复现 M7 结果  
3. 预热池按 **`math`** 命中/未命中可观测；无双分配  
4. 日志含 `env_type=math`；可选 `dataset=gsm8k` 于 payload 或 span attribute  
5. Protobuf/UDS 子进程通信正常；插件 crash 不拖垮 Worker（§6.4）  
6. Worker `RegisterWorker.supported_env_types` 上报 **`["math"]`**，与 Hub manifest 一致  

### 9.4 从 `gsm8k` env_type 迁移清单（Phase 0+）
...
| env_type | 说明 | 与 math 关系 |
|----------|------|--------------|
| `code` | CodeEnv 执行型 | 独立插件与预热池 |
| `agent` | AgentEnv 交互型 | 独立插件与预热池 |
| math 内新 benchmark | 如 `dataset: math500` | 仅增 backend，不增 env_type |

---

## 9-legacy. ~~Phase 0 范围：仅 GSM8K~~（已 supersede）

> v1.3 及以前版本：Phase 0 以 `env_type=gsm8k` 为调度键。**v1.4 起由 §9 MathEnv 取代**；保留本节标题仅供 diff 追溯，请勿在新代码/文档中引用。
...
| `UENV_ENV_TYPES` | `env.types` | string | `math` | **Phase 0 仅 math（MathEnv）**（YAML 为数组） |
...
plugins/math/                   # Phase 0 MathEnv（gsm8k 为 backends/gsm8k）
├── manifest.yaml
├── backends/gsm8k/
└── ...
plugins/gsm8k/                  # [deprecated v1.4] 迁移至 math/backends/gsm8k
```
...
| **W1** | `PluginHost` + **MathEnv** 插件（ProcessBackend；gsm8k backend） | MathEnv 环境实现 |
...
| Phase 0 环境 | 多种 | 仅 GSM8K | **MathEnv（`env_type=math`）；GSM8K 为 benchmark** |
...
- [UEnv 设计 PRD v7.2](./uenv-design-prd-v7.2.md) — §3.2.5 内置环境矩阵、§8.1 MathEnv
