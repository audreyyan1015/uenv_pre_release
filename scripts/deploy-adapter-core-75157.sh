...
sudo mkdir -p /var/lib/uenv/trajectory-data/bodies /var/log/uenv
sudo chown -R "$(whoami)" /var/lib/uenv/trajectory-data /var/log/uenv 2>/dev/null || true
...
sleep 2
nohup "$UENV_HOME/target/release/uenv-adapter-core" >> /var/log/uenv/adapter-core.log 2>&1 &
sleep 4
...
tail -20 /var/log/uenv/adapter-core.log | grep -E 'trajectory|listening|8088' || tail -5 /var/log/uenv/adapter-core.log
