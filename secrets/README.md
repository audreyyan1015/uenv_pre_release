...
| 主机 | 云厂商 | 角色 | 公网 IP | 内网 IP | 系统盘（`/`） | SSH 登录 | 已开放公网 TCP 端口 | 关键 endpoint |
|------|--------|------|---------|---------|---------------|----------|---------------------|---------------|
| **7142** | —（A100 VM） | VeRL / Python Adapter | `219.147.100.43` | `10.10.20.142` | **850 GB** NVMe | 端口 **7142**，私钥 + `root`（§4） | **7142**（SSH），**18000**，**18077**，**18088**，**18099**，**18777**，**18888** | 出站 → **`8.130.75.157:8088`** |
| **7143** | —（A100 VM） | uenv-worker | `219.147.100.43` | `10.10.20.143` | **850 GB** NVMe | 端口 **7143**，私钥 + `root`（§4） | **7143**（SSH），**28000**，**28077**，**28088**，**28097**，**28099**，**28777**，**28888** | gRPC **`219.147.100.43:28888`**；health **`219.147.100.43:28777`**；Runtime Gateway **`219.147.100.43:28097`** |
| **Server** | **阿里云** | uenv-adapter-core + Server | **`8.130.75.157`** | — | **252 GB** NVMe | `root` / `dev@BDW2026` | **22**（SSH），**8000**，**8077**，**8088**，**8099** | gRPC **`8.130.75.157:8088`** |
| **Hub** | **阿里云** | uenv-hub | **`8.130.95.176`** | `192.168.0.133`（同 VPC） | **99 GB** NVMe | `root` / `pku@345`；`pku` / `pku@123` | **22**（SSH），**8000**，**8077**，**8088**，**8099** | REST **`http://8.130.95.176:8088`** |
| **阿里云 16C64G** | **阿里云** | 扩展 / 备用 | **`121.89.82.128`** | — | **504 GB** NVMe | `root` / `dev@BDW2026` | **22**（SSH），**5432**，**6379**，**8000**，**8077**，**8088**，**8099**，**8777**，**8888** | — |
| **阿里云 8C32G** | **阿里云** | **Agent 池**（OpenHands + `uenv-agent-openhands` bridge）/ SWE Benchmark | **`8.130.208.77`** | — | **252 GB** NVMe | `root` / `dev@BDW2026` | **22**（SSH），**5432**，**6379**，**8000**，**8077**，**8088**，**8099**，**8777**，**8888** | Runner **`8.130.208.77:8888`**；health **`8.130.208.77:8777`**；出站 **`8.130.75.157:8088`**（AgentControl，见 §3.8） |

> 系统盘为容量配置；占用率与剩余空间见 **§1.4** / **§2**（2026-06-27 实机采集）。
