...
按照 Worker 侧五类 benchmark 文档，SciTab 复用通用验证型 `qa` 环境，由 Worker 内部根据 `env_config.dataset=scitab` 路由到对应判分逻辑。
...
GATEWAY_LOG_DIR=/data/ronghao/uenv/uenv-bridge/temp/benchmarks/scitab/gateway-$(date +%Y%m%d-%H%M%S)
...
RUN_ID=scitab-full-$(date +%Y%m%d-%H%M%S)
OUT=/data/ronghao/uenv/uenv-bridge/temp/benchmarks/scitab/${RUN_ID}
mkdir -p "$OUT"
...
./scripts/benchmark/scitab/run_scitab_uenv_baseline.sh 2>&1 | tee "$OUT/run.log"
```
...
|---|---|---|---:|---:|---:|---:|---:|---:|---:|
| `Qwen/Qwen3.6-35B-A3B` | `8.130.75.157:8088` | `http://10.10.20.142:18094/v1` | 1224 | 1224 | 0 | 1.0000 | 0.7451 | 0.7340 | 0.7451 |
