...
| 技术 | VeRL | ROLL | ROCK |
|---|---|---|---|
| 同步 step | 当前主线，语义清楚，rollout 完成后再 update | 预计支持同步 PPO/GRPO，可作为对照基线 | 不直接负责 trainer，同步语义取决于上层训练框架 |
| One-step off-policy | experimental；一拍流水线，下一批 rollout 与当前 update 重叠 | 需要确认是否有等价配置，重点看 off-policy loss、IS correction、parameter sync | ROCK 本身不承担 trainer update，若与 ROLL 组合则由 ROLL 控制 |
| Fully async | experimental；rollouter 和 trainer 通过 queue 解耦，有 staleness/queue 指标 | 重点调研对象，官方特性中提到 asynchronous rollout/training | ROCK 可能提供 environment/sandbox 并发，但 trainer consumer 仍依赖 ROLL 等训练框架 |
| Rollout 多 endpoint | VeRL 可启动多个 vLLM endpoint；当前 UEnv 需要 model gateway 做统一入口 | ROLL 需要看 rollout scheduler 和 AutoDeviceMapping 如何管理 endpoint | ROCK 更可能管理环境 worker，而不是 vLLM endpoint |
| Policy version | VeRL async/one-step 需要关注 `global_steps`、rollout log prob、权重同步 | 需要确认是否显式记录 policy version、staleness、IS 权重 | ROCK 层应透传 metadata，不应丢失 request/result 归属 |
| Environment 并发 | AgentLoop 内可并发，但当前 UEnv 主要瓶颈在外部 Worker/model generation | 可能支持 environment-level async rollout | ROCK 的优势可能在这里：环境、sandbox、GEM 调度 |

### 3.3 能力矩阵

| 维度 | VeRL | ROLL | ROCK |
|---|---|---|---|
| 是否是训练框架 | 是 | 是 | 不是主训练框架，更偏环境框架 |
| 是否支持 GRPO/PPO | 是 | 是 | 依赖外部训练框架 |
| 是否支持 rollout/train overlap | experimental 支持 | 需要通过脚本验证 | 不是主职责 |
| 是否支持 fully async | experimental 支持 | 重点验证 | 不是主职责 |
| 是否支持 environment async | AgentLoop/rollout 侧可扩展 | 需要验证 | 是重点能力方向 |
| 是否管理 vLLM/SGLang endpoint | 是，VeRL rollout 侧启动 endpoint | 是，需看 scheduler 和 device mapping | 通常不直接管理模型 endpoint |
| 是否需要 staleness 控制 | one-step/fully async 需要 | 重点验证 | 应透传 metadata |
| 是否需要 IS/off-policy correction | one-step/async 下需要关注 | 重点验证 | 不直接处理 |
| 对 UEnv Adapter 的直接启发 | policy version、batch、gateway、异步 result buffer | rollout scheduler、AutoDeviceMapping、异步训练策略 | environment/GEM 协议和 sandbox 调度 |
...
| Stage 5 | 统一资源切分对照：4GPU、8GPU | 三框架对比结论 |
...
本节记录 2026-06-24 对 ROLL 的最小复现实验。目标是先确认 ROLL 在当前镜像和本机资源下能跑通三类调度形态，再决定是否做更长步数和更大资源切分实验。
...
| `scripts/roll_step_parallel/configs/roll_agentic_async_rollout_frozenlake.yaml` | ROLL Agentic async rollout smoke 配置 |
...
| async rollout | 使用 ROLL Agentic pipeline + FrozenLake 内置环境验证 environment-level async rollout |
| Atropos/GSM8K async rollout | 当前没有完成，阻塞在缺少 `atroposlib` / `environments.gsm8k_server` 依赖 |
| 指标可比性 | 本节是 1-step smoke，重点证明机制可运行；由于 sync/async training 与 async rollout 使用不同 pipeline 和任务，不直接比较训练效果 |
...
| ROLL agentic async rollout | Agentic | FrozenLake | 完成 | `temp/logs/roll_step_parallel/agentic_async_rollout_frozenlake/roll_agentic_async_rollout_frozenlake_fsdp2_20260624_135901.log` |

关键指标：

| 指标 | 说明 |
|---|---|
| `pipeline complete!` | 三种模式均出现 |
| `metrics_tag` | 三种模式均输出 |
...
| Agentic async rollout `time/step_rollout` | 4.1615s |
| Agentic async rollout `time/step_train` | 5.1104s |
| Agentic async rollout `time/step_total` | 15.5759s |
...
| Agentic async rollout reward 为 0 | FrozenLake smoke 中模型没有输出有效动作，环境给出 format penalty；本实验关注 async rollout 链路，不评估任务效果 |
...
| Atropos/GSM8K async rollout 缺依赖 | 已确认缺 `atroposlib` / `environments.gsm8k_server`，本轮改用 ROLL 内置 FrozenLake 验证 async rollout 机制 |
...
| ROLL 三类 smoke 已能跑通 | 已复现 sync、async training、Agentic async rollout，但目前只是 1-step smoke |
...
| 第五阶段 | 决定 UEnv Adapter 是否需要引入 policy_version、异步 result buffer、staleness check |
