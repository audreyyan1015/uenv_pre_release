...
Usage:
  SERVER_ADAPTER_CORE_ENDPOINT=<server-core-host:port> ./scripts/train/launchers/common/run_verl_uenv_grpo.sh
...
Common environment overrides:
  ...
  TRAIN_SEED                    Shared seed for data order, loaders, and rollout RNG. Default: 42
  DATA_SHUFFLE                  Data sampler shuffle flag. Default: false
  DATA_SEED                     Data sampler seed when shuffling is enabled. Default: TRAIN_SEED
  ACTOR_DATA_LOADER_SEED        Actor-side mini-batch seed. Default: TRAIN_SEED
  CRITIC_DATA_LOADER_SEED       Critic-side mini-batch seed. Default: TRAIN_SEED
  ROLLOUT_SEED                  vLLM rollout seed. Default: TRAIN_SEED
  ...
  UENV_AGENT_POOL_ID             Optional server-side agent pool override for SWE/OpenHands episodes.
...
ACTOR_DATA_LOADER_SEED=${ACTOR_DATA_LOADER_SEED:-${TRAIN_SEED}}
CRITIC_DATA_LOADER_SEED=${CRITIC_DATA_LOADER_SEED:-${TRAIN_SEED}}
...
UENV_AGENT_POOL_ID=${UENV_AGENT_POOL_ID:-}
VERL_AGENT_LOOP_NAME=${VERL_AGENT_LOOP_NAME:-uenv_agent}
VERL_AGENT_LOOP_CONFIG_PATH=${VERL_AGENT_LOOP_CONFIG_PATH:-/uenv/uenv-bridge/configs/uenv-agent-loop.yaml}
...
write_json_metadata "${CHECKPOINT_RUN_DIR}/metadata.json" \
  ...
  "critic_data_loader_seed=${CRITIC_DATA_LOADER_SEED}" \
  "rollout_seed=${ROLLOUT_SEED}" \
  "agent_pool_id=${UENV_AGENT_POOL_ID}" \
...
PODMAN_UENV_MONOREPO_ARGS=()
if [ "${UENV_MONOREPO_MOUNT_ENABLED:-0}" = "1" ]; then
  UENV_MONOREPO_DIR=${UENV_MONOREPO_DIR:-"$(cd "${REPO_DIR}/.." && pwd)"}
  PODMAN_UENV_MONOREPO_ARGS=(-v "${UENV_MONOREPO_DIR}:/uenv")
fi
PODMAN_NATIVE_SWE_SSH_ARGS=()
if [ -n "${NATIVE_SWE_REMOTE_IDENTITY_FILE_HOST:-}" ]; then
  NATIVE_SWE_REMOTE_IDENTITY_FILE_CONTAINER=${NATIVE_SWE_REMOTE_IDENTITY_FILE_CONTAINER:-/run/uenv-native-swe/ssh_key}
  if [ ! -f "${NATIVE_SWE_REMOTE_IDENTITY_FILE_HOST}" ]; then
    echo "NATIVE_SWE_REMOTE_IDENTITY_FILE_HOST not found: ${NATIVE_SWE_REMOTE_IDENTITY_FILE_HOST}" >&2
    exit 1
  fi
  PODMAN_NATIVE_SWE_SSH_ARGS=(-v "${NATIVE_SWE_REMOTE_IDENTITY_FILE_HOST}:${NATIVE_SWE_REMOTE_IDENTITY_FILE_CONTAINER}:ro")
  NATIVE_SWE_REMOTE_IDENTITY_FILE=${NATIVE_SWE_REMOTE_IDENTITY_FILE:-${NATIVE_SWE_REMOTE_IDENTITY_FILE_CONTAINER}}
fi
if [ -n "${NATIVE_SWE_REMOTE_PASSWORD:-}" ]; then
  NATIVE_SWE_SSHPASS_HOST=${NATIVE_SWE_SSHPASS_HOST:-$(command -v sshpass || true)}
  NATIVE_SWE_SSHPASS_CONTAINER=${NATIVE_SWE_SSHPASS_CONTAINER:-/usr/local/bin/sshpass}
  if [ -z "${NATIVE_SWE_SSHPASS_HOST}" ] || [ ! -f "${NATIVE_SWE_SSHPASS_HOST}" ]; then
    echo "ssh password backend requires sshpass on host; set NATIVE_SWE_SSHPASS_HOST or use a key" >&2
    exit 1
  fi
  PODMAN_NATIVE_SWE_SSH_ARGS+=(-v "${NATIVE_SWE_SSHPASS_HOST}:${NATIVE_SWE_SSHPASS_CONTAINER}:ro")
fi
...
run_verl_training() {
  ...
  if [ "${VERL_AGENT_LOOP_NAME:-uenv_agent}" = "native_swe_agent" ] && [ "${NATIVE_SWE_EXECUTION_BACKEND:-local}" = "grpc" ]; then
    echo "Native AgentControl endpoint: ${NATIVE_SWE_AGENT_CONTROL_PUBLIC_ENDPOINT:-${NATIVE_SWE_AGENT_CONTROL_HOST:-0.0.0.0}:${NATIVE_SWE_AGENT_CONTROL_PORT:-19051}}"
  fi
...
export UENV_AGENT_POOL_ID=\"${UENV_AGENT_POOL_ID}\"
...
export VERL_AGENT_LOOP_NAME=${VERL_AGENT_LOOP_NAME:-uenv_agent}
export VERL_AGENT_LOOP_CONFIG_PATH=${VERL_AGENT_LOOP_CONFIG_PATH:-/uenv/uenv-bridge/configs/uenv-agent-loop.yaml}
export NATIVE_SWE_DRIVER_PATH=\"${NATIVE_SWE_DRIVER_PATH:-}\"
export NATIVE_SWE_PYTHON=\"${NATIVE_SWE_PYTHON:-python3}\"
export NATIVE_SWE_OUTPUT_ROOT=\"${NATIVE_SWE_OUTPUT_ROOT:-}\"
export NATIVE_SWE_EXECUTION_BACKEND=\"${NATIVE_SWE_EXECUTION_BACKEND:-local}\"
export NATIVE_SWE_RUNTIME_GATEWAY_URL=\"${NATIVE_SWE_RUNTIME_GATEWAY_URL:-}\"
export NATIVE_SWE_RUNTIME_GATEWAY_API_KEY=\"${NATIVE_SWE_RUNTIME_GATEWAY_API_KEY:-}\"
export NATIVE_SWE_LLM_CONFIG_PATH=\"${NATIVE_SWE_LLM_CONFIG_PATH:-}\"
export NATIVE_SWE_INSTANCES_CATALOG=\"${NATIVE_SWE_INSTANCES_CATALOG:-}\"
...
export NATIVE_SWE_REMOTE_HOST=\"${NATIVE_SWE_REMOTE_HOST:-}\"
export NATIVE_SWE_REMOTE_USER=\"${NATIVE_SWE_REMOTE_USER:-root}\"
export NATIVE_SWE_REMOTE_PORT=\"${NATIVE_SWE_REMOTE_PORT:-22}\"
export NATIVE_SWE_REMOTE_IDENTITY_FILE=\"${NATIVE_SWE_REMOTE_IDENTITY_FILE:-}\"
export NATIVE_SWE_REMOTE_PASSWORD=\"${NATIVE_SWE_REMOTE_PASSWORD:-}\"
...
export NATIVE_SWE_REMOTE_BRIDGE_DIR=\"${NATIVE_SWE_REMOTE_BRIDGE_DIR:-}\"
export NATIVE_SWE_REMOTE_LLM_CONFIG_PATH=\"${NATIVE_SWE_REMOTE_LLM_CONFIG_PATH:-}\"
export NATIVE_SWE_REMOTE_SSH_EXTRA_ARGS=\"${NATIVE_SWE_REMOTE_SSH_EXTRA_ARGS:-}\"
...
export NATIVE_SWE_AGENT_CONTROL_PUBLIC_ENDPOINT=\"${NATIVE_SWE_AGENT_CONTROL_PUBLIC_ENDPOINT:-}\"
python3 /uenv/uenv-bridge/scripts/train/wrappers/run_verl_main_ppo.py \\
  ...
  actor_rollout_ref.rollout.agent.default_agent_loop=${VERL_AGENT_LOOP_NAME} \\
  actor_rollout_ref.rollout.agent.agent_loop_config_path=${VERL_AGENT_LOOP_CONFIG_PATH} \\
  ...
  critic.data_loader_seed=${CRITIC_DATA_LOADER_SEED} \\
  ...
  +ray_kwargs.ray_init.runtime_env.env_vars.UENV_AGENT_POOL_ID=\\\"${UENV_AGENT_POOL_ID}\\\" \\
