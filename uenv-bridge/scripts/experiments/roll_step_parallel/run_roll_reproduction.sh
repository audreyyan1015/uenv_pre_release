...
Examples:
  ROLL_MODE=sync ROLL_MAX_STEPS=1 ./scripts/experiments/roll_step_parallel/run_roll_reproduction.sh
  ROLL_MODE=async_training ROLL_MAX_STEPS=1 ./scripts/experiments/roll_step_parallel/run_roll_reproduction.sh
  ROLL_MODE=agentic_async_rollout ROLL_MAX_STEPS=1 PODMAN_GPU_ARGS="nvidia.com/gpu=0,1" CUDA_VISIBLE_DEVICES_IN_CONTAINER=0,1 ./scripts/experiments/roll_step_parallel/run_roll_reproduction.sh
  ROLL_MODE=agentic_async_rollout_frozenlake ROLL_MAX_STEPS=1 PODMAN_GPU_ARGS="nvidia.com/gpu=0,1" CUDA_VISIBLE_DEVICES_IN_CONTAINER=0,1 ./scripts/experiments/roll_step_parallel/run_roll_reproduction.sh
EOF
}
...
ensure_path "${REPO_DIR}/configs/roll_step_parallel/${ROLL_CONFIG_NAME}.yaml" "ROLL config not found"
...
podman run --rm \
  ...
  "${IMAGE}" \
  -lc "set -euo pipefail
export PYTHONPATH=/uenv/uenv-bridge/scripts/experiments/roll_step_parallel:/workspace/ROLL-main:/workspace/ROLL-main/mcore_adapter/src:\${PYTHONPATH:-}
...
python3 scripts/experiments/roll_step_parallel/start_roll_pipeline.py \
  --config-dir /uenv/uenv-bridge/configs/roll_step_parallel \
