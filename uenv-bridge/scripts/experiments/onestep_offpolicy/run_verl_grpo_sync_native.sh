...
usage() {
  cat <<'EOF'
Run native synchronous VeRL GRPO inside the VeRL podman image.

This is the native VeRL baseline for comparing with
run_verl_grpo_onestep_offpolicy.sh. It does not connect to UEnv Server/Worker,
and it does not enable UEnvAgentLoop.

The defaults intentionally mirror the one-step off-policy script where the
synchronous VeRL trainer accepts the same knobs.

Usage:
  ./scripts/experiments/onestep_offpolicy/run_verl_grpo_sync_native.sh
...
DEFAULT_RAY_NUM_CPUS=$((NGPUS_PER_NODE * 4))
RAY_NUM_CPUS=${RAY_NUM_CPUS:-${DEFAULT_RAY_NUM_CPUS}}
# Match the stable one-step behavior: let Ray isolate each GPU actor.
...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-uenv_sync_native_$(date +%Y%m%d_%H%M)}
RUN_ID=${RUN_ID:-sync_native_$(date +%Y%m%d_%H%M%S)}

LOG_ROOT=${LOG_ROOT:-${REPO_DIR}/temp/logs}
CONTAINER_LOG_ROOT=${CONTAINER_LOG_ROOT:-/uenv/uenv-bridge/temp/logs}
LOG_DIR=${LOG_DIR:-${LOG_ROOT}/verl_sync_native}
...
echo "Running native synchronous VeRL GRPO; log: ${LOG_FILE}"
echo "GPU layout: trainer/rollout share ${NGPUS_PER_NODE} visible GPUs; rollout_tp=${ROLLOUT_TP}"
...
python3 -m verl.trainer.main_ppo \\
  hydra.run.dir=${CONTAINER_LOG_ROOT}/verl_sync_native/hydra_${RUN_ID} \\
  ...
  trainer.project_name=uenv_bridge_sync_native \\
  ...
  trainer.default_local_dir=/uenv/uenv-bridge/tmp/verl_sync_native_ckpt \\
...
if [ "${run_status}" -ne 0 ]; then
  echo "Native synchronous VeRL run failed. Log: ${LOG_FILE}" >&2
  tail -120 "${LOG_FILE}" >&2 2>/dev/null || true
  exit "${run_status}"
fi

echo "Native synchronous VeRL run completed."
echo "Log: ${LOG_FILE}"
