#!/usr/bin/env bash
# Prepare and run a pinned VeRL GRPO example through UEnv SWE environments.
set -euo pipefail

VERL_REPOSITORY="https://github.com/verl-project/verl.git"
VERL_VERSION="v0.7.1"
VERL_COMMIT="bec9ef74768dd201881cd4e54cd0385e87caae27"
DEFAULT_IMAGE="docker.io/verlai/verl:vllm017.latest"
...
GPU 主机（准备固定版本 VeRL 和 Bridge）：
  bash train_verl.sh prepare-gpu --bundle ./uenv-linux-x86_64.tar.gz

准备数据：
  bash train_verl.sh prepare-data \
    --catalog /opt/uenv/current/share/swe/smith-example.json \
    --output-dir ./swe-data --limit 1

单机运行：
  bash train_verl.sh run --uenv-release /opt/uenv/current \
    --model /path/to/model --data ./swe-data

双机运行（在 GPU 主机）：
  bash train_verl.sh run --bundle ./uenv-linux-x86_64.tar.gz \
    --model /path/to/model --data ./swe-data \
...
prepare-data 选项：
  ...
  --limit N                最多选择 N 个实例；0 表示全部（默认 1）
  --instance ID            只选择指定实例，可重复
  --max-iterations N       每条轨迹最大 Agent 步数（默认 10）
  --runtime docker|podman  主机缺 pyarrow 时使用的容器运行时
  --image IMAGE            数据准备容器（默认与训练镜像相同）

run 选项：
  --model DIR              Hugging Face 模型目录（必填）
  --data DIR               含 train.parquet/test.parquet 的目录（必填）
  --uenv-endpoint HOST:PORT  UEnv adapter-core 地址（默认 127.0.0.1:50051）
  --gateway-public-url URL CPU/UEnv 主机可访问的 GPU 模型网关 URL
  --gateway-port PORT      GPU 模型网关监听端口（默认 18080）
  --gpus N                 单节点 GPU 数（默认 1）
  --steps N                训练步数（默认 1）
  --rollouts N             每个问题的轨迹数（默认 2）
  --train-batch-size N     每批问题数（默认 1）
  --runtime docker|podman  容器运行时（默认自动选择）
  --image IMAGE            VeRL CUDA 镜像
...
prepare_uenv() {
  ...
  [[ -f "$release/share/swe/smith-example.json" ]] || fail "release 缺少 SWE-smith 示例 catalog"
  ...
  local swe_runtime image
  swe_runtime="$(awk -F= '$1 == "UENV_SWE_RUNTIME" {print substr($0, length($1) + 2); exit}' /etc/uenv/swe.env)"
  [[ "$swe_runtime" == docker || "$swe_runtime" == podman ]] \
    || fail "/etc/uenv/swe.env 中的 UENV_SWE_RUNTIME 必须是 docker 或 podman"
  require_command "$swe_runtime"
  require_command runuser
  image="$(python3 - "$release/share/swe/smith-example.json" <<'PY'
import json
import sys
...
rows.sort(key=lambda row: str(row.get("instance_id") or "") if isinstance(row, dict) else "")
for row in rows:
    if isinstance(row, dict):
        image = str(row.get("image_cache_key") or row.get("image_name") or "").strip()
        if image:
            print(image if ":" in image.rsplit("/", 1)[-1] else image + ":latest")
            raise SystemExit(0)
raise SystemExit("Smith example catalog contains no container image")
PY
)"
  if ! runuser -u uenv -- "$swe_runtime" image inspect "$image" >/dev/null 2>&1; then
    [[ "$offline" -eq 0 ]] \
      || fail "本机没有示例环境镜像 $image；去掉 --offline 后重试"
    echo "==> 下载示例环境镜像 $image"
    runuser -u uenv -- "$swe_runtime" pull "$image"
  else
    echo "==> 示例环境镜像已就绪"
  fi
...
ensure_verl_checkout() {
  ...
  [[ "$actual" == "$VERL_COMMIT" ]] \
    || fail "$checkout 不是 $VERL_VERSION 固定提交；请使用新的 --work-dir"
  printf '%s\n' "$checkout"
}
...
prepare_data() {
  local catalog="" output_dir="" limit=1 max_iterations=10 runtime="" image="$DEFAULT_IMAGE"
  local -a instances=()
  while (($#)); do
    case "$1" in
  ...
  if [[ -z "$runtime" ]] && ! command -v docker >/dev/null 2>&1 && ! command -v podman >/dev/null 2>&1; then
    fail "主机缺少 pandas/pyarrow 且没有容器运行时；请安装：python3 -m pip install pandas pyarrow"
  fi
...
run_training() {
  parse_asset_options
  local model="" data="" endpoint="127.0.0.1:50051" gateway_url="" gateway_port=18080
  local gpus=1 steps=1 rollouts=2 train_batch=1 runtime="" image="$DEFAULT_IMAGE" dry_run=0
  local -a extra_hydra=()
  while (($#)); do
    case "$1" in
      --bundle) BUNDLE="${2:-}"; shift 2 ;;
  ...
  local -a hydra_args=(
    ...
    "trainer.project_name=uenv_swe"
    "trainer.experiment_name=uenv_swe_grpo"
...
export UENV_MODEL_GATEWAY_BIND_HOST=0.0.0.0
